...
if (life_action_inUse) exitWith {[localize "STR_NOTF_ActionInProc",true,"slow"] call ALSG_fnc_notificationSystem;};
if !(playerSide isEqualTo civilian) exitWith {[localize "STR_NOTF_notAllowed",true,"slow"] call ALSG_fnc_notificationSystem;};
...
if (count _nearUnits > 1) exitWith {[localize "STR_NOTF_PlayerNear",true,"slow"] call ALSG_fnc_notificationSystem;};

life_chopShop = _this select 3;
//Error check
if (count _nearVehicles isEqualTo 0) exitWith {titleText[localize "STR_Shop_NoVehNear","PLAIN"];};
if (!(createDialog "Chop_Shop")) exitWith {[localize "STR_Shop_ChopShopError",true,"slow"] call ALSG_fnc_notificationSystem;};
