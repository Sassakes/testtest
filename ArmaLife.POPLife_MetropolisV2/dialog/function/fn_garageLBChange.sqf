...
_trunkSpace = [_className] call ALSG_fnc_vehicleWeightCfg;
...
switch (playerSide) do {
    case civilian: {
        ...
        _sellMultiplier = getNumber(_shopSettings >> "vehicle_sell_multiplier_CIVILIAN");
    };
    case west: {
        _purchasePrice = _price * getNumber(_shopSettings >> "vehicle_purchase_multiplier_COP");
        _sellMultiplier = getNumber(_shopSettings >> "vehicle_sell_multiplier_COP");
    };
    case independent: {
        _purchasePrice = _price * getNumber(_shopSettings >> "vehicle_purchase_multiplier_MEDIC");
        _sellMultiplier = getNumber(_shopSettings >> "vehicle_sell_multiplier_MEDIC");
    };
};

_retrievePrice = _purchasePrice * getNumber(_shopSettings >> "vehicle_storage_fee_multiplier");
...
(CONTROL(2800,2803)) ctrlSetStructuredText parseText format [
    ...
    ",
[_retrievePrice] call ALSG_fnc_numberText,
[_sellPrice] call ALSG_fnc_numberText,
