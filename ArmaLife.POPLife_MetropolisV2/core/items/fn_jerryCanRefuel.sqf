...
if (count(nearestObjects [player,["Land_FuelStation_Feed_F","Land_fs_feed_F"],3.5]) isEqualTo 0) exitWith {[localize "STR_ISTR_Jerry_Distance",true,"slow"] call ALSG_fnc_notificationSystem;};
...
private _action = [localize "STR_ISTR_Jerry_StationPump", format[localize "STR_ISTR_Jerry_PopUp",[_fuelCost] call ALSG_fnc_numberText], "YES", "NO", findDisplay 46, true] call ALSG_fnc_guiPrompt;

if (_action) then {
    if (CASH < _fuelCost) exitWith {[localize "STR_NOTF_NotEnoughMoney",true,"slow"] call ALSG_fnc_notificationSystem; life_action_inUse = false;};
    ...
    for "_i" from 0 to 1 step 0 do {
        if !((animationState player) == "AinvPknlMstpSnonWnonDnon_medic_1") then {
            [player,"AinvPknlMstpSnonWnonDnon_medic_1",true] remoteExecCall ["ALSG_fnc_animSync",RCLIENT];
    ...
    if (!([false,"fuelEmpty",1] call ALSG_fnc_handleInv)) exitWith {life_action_inUse = false;};
    ...
    [_fuelCost,true] call ArmaLife_AC_Cash;

    [true,"fuelFull",1] call ALSG_fnc_handleInv;
    [localize "STR_ISTR_Jerry_Refueled",true,"slow"] call ALSG_fnc_notificationSystem;
} else {
    [localize "STR_NOTF_ActionCancel",true,"slow"] call ALSG_fnc_notificationSystem;
    closeDialog 0;
    life_action_inUse = false;
};