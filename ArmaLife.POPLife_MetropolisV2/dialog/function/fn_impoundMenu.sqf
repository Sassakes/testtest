...
if (_vehicles isEqualTo []) exitWith {
	["You do not have any vehicles in the garage.",true,"slow"] call ALSG_fnc_notificationSystem;
	closeDialog 0;
};
...
_ctrlFooterRetrieveVehicle ctrlAddEventHandler ["ButtonClick", {[] call ALSG_fnc_unimpound;}];
_ctrlFooterSellVehicle ctrlAddEventHandler ["ButtonClick", {closeDialog 0; [] call ALSG_fnc_sellGarage;}];
_ctrlFooterInsureVehicle ctrlAddEventHandler ["ButtonClick", {_this spawn ALSG_fnc_insureVehicle;}];
...

{
	...
	if (isClass (configFile >> "CfgVehicles" >> _vehicleClass)) then {
		...
		private _vehicleInfo = [_vehicleClass] call ALSG_fnc_fetchVehInfo;
