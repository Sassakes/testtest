...
if(player distance _vehicle > 5) exitWith {["You have to be within 5m of the vehicle.",true,"slow"] call ALSG_fnc_notificationSystem;};
if(!([false,"enginedisable",1] call ALSG_fnc_handleInv)) exitWith {["You need a Engine Disabler.",true,"slow"] call ALSG_fnc_notificationSystem;};
...
["You have placed the egine disabler on the vehicle.",false,"slow"] call ALSG_fnc_notificationSystem;
player addAction [format["<t color='#4DB0E2'>Disable Engine (%1)",_displayName], {_veh = (_this select 3); if(!isNull _veh && alive _veh) then {[_veh] remoteExec ["ALSG_fnc_setEngineHit",_veh]; }; (_this select 0) removeAction (_this select 2);}, _vehicle,0,true,true];