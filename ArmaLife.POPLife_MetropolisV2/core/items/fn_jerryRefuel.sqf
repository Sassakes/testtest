...
if (isNull _vehicle) exitWith {[localize "STR_ISTR_Jerry_NotLooking",true,"slow"] call ALSG_fnc_notificationSystem;};
if (!(_vehicle isKindOF "LandVehicle") && !(_vehicle isKindOf "Air") && !(_vehicle isKindOf "Ship")) exitWith {};
if (player distance _vehicle > 7.5) exitWith {[localize "STR_ISTR_Jerry_NotNear",true,"slow"] call ALSG_fnc_notificationSystem;};

if (!([false,"fuelFull",1] call ALSG_fnc_handleInv)) exitWith {};
...
if (!alive player) exitWith {};
if (life_interrupted) exitWith {life_interrupted = false; [localize "STR_NOTF_ActionCancel",false,"slow"] call ALSG_fnc_notificationSystem;};
...
switch (true) do {
    case (_vehicle isKindOF "LandVehicle"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.3] remoteExecCall ["ALSG_fnc_setFuel",_vehicle];
        } else {
    ...
    case (_vehicle isKindOf "Air"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.1] remoteExecCall ["ALSG_fnc_setFuel",_vehicle];
        } else {
    ...
    case (_vehicle isKindOf "Ship"): {
        if (!local _vehicle) then {
            [_vehicle,(Fuel _vehicle) + 0.35] remoteExecCall ["ALSG_fnc_setFuel",_vehicle];
        } else {
            _vehicle setFuel ((Fuel _vehicle) + 0.35);
        };
    };
};
[format[localize "STR_ISTR_Jerry_Success",_displayName],false,"slow"] call ALSG_fnc_notificationSystem;
[true,"fuelEmpty",1] call ALSG_fnc_handleInv;