...
if (_isVehicle && _curObject in life_vehicles) exitWith {[localize "STR_ISTR_Lock_AlreadyHave",true,"slow"] call ALSG_fnc_notificationSystem;};

//More error checks
if (!_isVehicle && !isPlayer _curObject) exitWith {};
if (!_isVehicle && !(_curObject getVariable ["restrained",false])) exitWith {};
if (_curObject getVariable "NPC") exitWith {[localize "STR_NPC_Protected",true,"slow"] call ALSG_fnc_notificationSystem;};
private _owners = _curObject getVariable ['vehicle_info_owners', []];
private _owner = [((_owners select 0) select 0)] call ArmaLife_Lib_GetPlayerByUID;
if (!(isNull _owner) && (side _owner) isEqualTo independent ) exitWith {["Did you know that medics are here to help? Stop being a braindead.",true,"slow"] call ALSG_fnc_notificationSystem;};
...
_chance = random (30);
if (_chance > 17) then {
	["Oh shit, the car alarm went off!",true,"slow"] call ALSG_fnc_notificationSystem;
...
if (!([false,"lockpick",1] call ALSG_fnc_handleInv)) exitWith {life_action_inUse = false;};
...
if (!_isVehicle) then {
    ...
    if (_dice < 30) then {
        ...
        [player,16] spawn ArmaLife_CREATEEVIDENCE;
    } else {
        [0,"STR_ISTR_Lock_FailedNOTF",true,[profileName]] remoteExecCall ["ALSG_fnc_broadcast",west];
