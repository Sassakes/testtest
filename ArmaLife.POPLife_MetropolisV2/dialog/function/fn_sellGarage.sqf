...
if ((time - life_action_delay) < 1.5) exitWith {[localize "STR_NOTF_ActionDelay",true,"slow"] call ALSG_fnc_notificationSystem;};
...
life_sellVehicle = [_sellPrice,_vid] spawn {
    ...
    private _action = ["Sell Vehicle", format["Are you sure you want to sell this vehicle for $%1? You can not undo this, it is permanent.",[_sellPrice] call ALSG_fnc_numberText], "YES", "NO", findDisplay 46, true] call ALSG_fnc_guiPrompt;
    closeDialog 0;

    if (_action) then {
        if ((time - life_action_delay) < 1.5) exitWith {[localize "STR_NOTF_ActionDelay",true,"slow"] call ALSG_fnc_notificationSystem; closeDialog 0;};
        life_action_delay = time;

        [_vid,getPlayerUID player,_sellPrice,player,life_garage_type] remoteExecCall ["TON_fnc_vehicleDelete",RSERV];

        [format[localize "STR_Garage_SoldCar",[_sellPrice] call ALSG_fnc_numberText],true,"slow"] call ALSG_fnc_notificationSystem;
        [_sellPrice] call ArmaLife_AC_Bank;
