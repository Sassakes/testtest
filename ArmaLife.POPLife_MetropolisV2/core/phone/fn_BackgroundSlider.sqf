if (ArmaLife_Phone_CurrentlyOpen !=  "ArmaLife_ChangeBackground") exitWith {};

while {!(ArmaLife_Phone_BackgroundControlMouseup)} do {
	...
	private _lastControl = ArmaLife_Phone_BackgroundControls select ((count ArmaLife_Phone_BackgroundControls) - 1);
	(ctrlPosition _lastControl) params ['','_lastControlTop','',''];
	private _firstControl = ArmaLife_Phone_BackgroundControls select 0;
	...
	if !(((_lastControlTop <= 0) && (_difference < 0)) || ((_firstControlBottom >= (_bottom - 0.09)) && (_difference > 0))) then {
		...
		(ArmaLife_Phone_BackgroundControlsButtons select _forEachIndex) ctrlSetPosition _pos;
		(ArmaLife_Phone_BackgroundControlsButtons select _forEachIndex) ctrlCommit 0;
		_x ctrlsetposition _pos;
		_x ctrlCommit 0;
		} forEach ArmaLife_Phone_BackgroundControls;
	};
