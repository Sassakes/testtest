...
if (count _containerInfo isEqualTo 0) exitWith {[localize "STR_Cop_ContainerEmpty",true,"slow"] call ALSG_fnc_notificationSystem;};
...
if (_value > 0) then {
    ...
    [0,"STR_NOTF_ContainerContraband",true,[[_value] call ALSG_fnc_numberText]] remoteExecCall ["ALSG_fnc_broadcast",RCLIENT];
    ...
    [parseText format["<t color='#FF0000'><t size='2'>This container contains contraband!</t></t><br/>%1",_contrabandInfo],false,"slow"] call ALSG_fnc_notificationSystem;

    _container setVariable ["Trunk",[[],0],true];
    [_container] remoteExecCall ["TON_fnc_updateHouseTrunk",2];
} else {
    ["This vehicle contains no illegal items",false,"slow"] call ALSG_fnc_notificationSystem;
    [localize "STR_Cop_NoIllegalContainer",true,"slow"] call ALSG_fnc_notificationSystem;
};
