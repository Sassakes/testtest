...
if (!isClass(missionConfigFile >> "VirtualShops" >> life_shop_type)) exitWith {closeDialog 0; [localize "STR_NOTF_ConfigDoesNotExist",true,"slow"] call ALSG_fnc_notificationSystem;}; //Make sure the entry exists..
...
// Loop through available items
{
    ...
    if (((toLower _displayName) find _searchFilter > -1 || (toLower (configname _configClass)) find _searchFilter > -1 || _searchFilter isEqualTo "") && _check) then {
        ...
        private _buyText = ["$" + ([_buyPrice] call ALSG_fnc_numberText), "None"] select (_buyPrice isEqualTo -1);
        private _sellText = ["$" + ([_sellPrice] call ALSG_fnc_numberText), "None"] select (_sellPrice isEqualTo -1);
