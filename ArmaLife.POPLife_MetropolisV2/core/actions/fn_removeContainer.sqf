...
if (!(_house in life_vehicles)) exitWith {[localize "STR_ISTR_Box_NotinHouse",true,"slow"] call ALSG_fnc_notificationSystem;};
if (isNull _container) exitWith {};
_containers = _house getVariable ["containers",[]];
closeDialog 0;

private _action = [localize "STR_pInAct_RemoveContainer", format[localize "STR_House_DeleteContainerMSG"], "YES", "NO", findDisplay 46, true] call ALSG_fnc_guiPrompt;
...
if (_action) then {
    ...
    _diff = [_box,1,life_carryWeight,life_maxWeight] call ALSG_fnc_calWeightDiff;
    if (_diff isEqualTo 0) exitWith {[localize "STR_NOTF_InvFull",true,"slow"] call ALSG_fnc_notificationSystem;};
    ...
    [true,_box,1] call ALSG_fnc_handleInv;
};
