  ...
  if (ArmaLife_Phone_CurrentlyOpen != "ArmaLife_MessagesView") then {
      ["ArmaLife_MessagesView"] spawn ALSG_fnc_changeMenu;
  };
  waitUntil {ArmaLife_Phone_CurrentlyOpen == "ArmaLife_MessagesView"};
  ...
  if (isNil "ArmaLife_MessageControls") then {
      ArmaLife_MessageControls = [];
  } else {
      if !(ArmaLife_MessageControls isEqualTo []) then {
          {
              ctrlDelete _x;
          } forEach ArmaLife_MessageControls;
      };
  };

  private _messageArrIndex = ArmaLife_Phone_Messages findIf {(_x select 0) isEqualTo _number};
  private _messageArr = if (_messageArrIndex isEqualTo -1) then {[_number,[],0]} else {(ArmaLife_Phone_Messages select _messageArrIndex)};
  ...
  if (_unreadMessages > 0) then {
      (ArmaLife_Phone_Messages select _messageArrIndex) set [2, 0];
  };
  ...
  _contactInfoBtn ctrlSetEventHandler ["ButtonClick", format ["['%1'] spawn ALSG_fnc_CheckSender", _number]];
  if ((_this select 1) isEqualTo 28) then {[]};
  private _btnSendMessage = _display displayCtrl 230008;
  _btnSendMessage ctrlSetEventHandler ["ButtonClick", format ["['%1'] call ALSG_fnc_SendMessage", _number]];
  private _messageBox = _display displayCtrl 230005;
  _messageBox ctrlSetEventHandler ["KeyDown",format ["if ((_this select 1) isEqualTo 28) then {['%1'] call ALSG_fnc_SendMessage}", _number]];
  {
      _x params ["_message", "_number"];

      private _imageFormat = if (_number isEqualTo (ArmaLife_Phone_profileSettings select 2)) then {"MessageTo"} else {"MessageFrom"};


      private _topCtrl = _display ctrlCreate ["ArmaLife_PhoneRscPicture", _someIDC, _controlGroup];                _someIDC = _someIDC + 1;
      private _midCtrl = _display ctrlCreate ["ArmaLife_PhoneRscPicture", _someIDC, _controlGroup];                _someIDC = _someIDC + 1;
      private _bottomCtrl = _display ctrlCreate ["ArmaLife_PhoneRscPicture", _someIDC, _controlGroup];             _someIDC = _someIDC + 1;
      private _structuredText =  _display ctrlCreate ["Life_RscStructuredText", _someIDC, _controlGroup];       _someIDC = _someIDC + 1;

      private _color = if (_number isEqualTo (ArmaLife_Phone_profileSettings select 2)) then {"#FFFFFF"} else {"#000000"};
      private _struct =  format ["<t align='left' color='%2' shadow='0' size='%3'>%1</t>", _message, _color,(((pixelH * 5) * ((getResolution select 1) / 1080)) * 100)];

      _bottomCtrl ctrlSetText format ["\ArmalifeCore\images\phone\ui\%1Bottom.paa",_imageFormat];
      _midCtrl ctrlSetText format ["\ArmalifeCore\images\phone\ui\%1Middle.paa",_imageFormat];
      _topCtrl ctrlSetText format ["\ArmalifeCore\images\phone\ui\%1Top.paa",_imageFormat];
      ...
      if ((count ArmaLife_MessageControls) > 0) then {
          private _lastControl = ArmaLife_MessageControls select ((count ArmaLife_MessageControls) - 1);
      ...
      _structuredTextPos set [
          0,
          if !(_number isEqualTo (ArmaLife_Phone_profileSettings select 2)) then {((_midCtrlPos select 2) * 0.035)} else {((_midCtrlPos select 2) * 0.315)}
      ];
      ...
      {ArmaLife_MessageControls pushBack _x} forEach [_structuredText, _topCtrl, _midCtrl, _bottomCtrl];
  } forEach _messages;
