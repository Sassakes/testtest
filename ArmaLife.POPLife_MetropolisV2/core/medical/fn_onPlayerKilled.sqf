...
[_killer,_unit] call ALSG_fnc_deathScreen;
...
if (!isNull _killer && {!(_killer isEqualTo _unit)} && {_killer getVariable ["killFeed",false]}) then {
    [name player] remoteExec ["ALSG_fnc_killfeed",_killer];
};

_espText = (_killer getVariable ["ArmaLife_AdminESP",'']);
if !(_espText isEqualTo "") then {
    [format["%1 has killed you with %2 on.",name _killer,_espText],true,"slow"] call ALSG_fnc_notificationSystem;
};
if (side _unit isEqualTo west) then {
    if ((player distance (getMarkerPos "jail_marker")) < 850) then {
        ...
        if ((time - _var) > 1800) then {
            ["If you were killed as a part of a major crime you can press Shift+P to declare it as one.",true,"slow"] call ALSG_fnc_notificationSystem;
        };
    };
};
...
life_hunger = 100;
life_thirst = 100;
[_unit,true] call ALSG_fnc_dropItems;
...
if (_cash > 0) then {
    ...
    [_cash, _pos] call ALSG_fnc_dropCash;
};
...
[_killer,_unit] spawn {
    ...
    if (!isNull _killer && {!(_killer isEqualTo _unit)} && {!(side _killer isEqualTo west)} && {alive _killer}) then {
        if (vehicle _killer isKindOf "LandVehicle") then {
            [_killer,7,life_corpse] spawn ArmaLife_CREATEEVIDENCE;
        } else {
            [_killer,7,life_corpse] spawn ArmaLife_CREATEEVIDENCE;            
        };
    };
