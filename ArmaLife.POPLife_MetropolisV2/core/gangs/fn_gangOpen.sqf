...
createDialog "ArmaLife_GangMenu";
...
private _freqs = missionNamespace getVariable ["ArmaLife_GangFreqs",[]];
...
_ctrlTreeGangs ctrlAddEventHandler ["TreeSelChanged", {
    ...
    if (_type isEqualTo 0) then {
        ...
        _ctrlGangBank ctrlSetText ("$" + ([_bank] call ALSG_fnc_numberText));

        private _ctrlTextOwner = _display displayCtrl 8999324;
        private _player = [_owner] call ArmaLife_Lib_GetPlayerByUID;
        ...
        if (_type isEqualTo 1) then {
            ...
            private _isOnline = _id call ArmaLife_Lib_GetPlayerStatus;
            private _status = [
                "Offline",
                "Online"
            ] select _isOnline;
            ...
            private _ctrlPromoteButton = _display displayCtrl 8999340;
            private _ctrlDemoteButton = _display displayCtrl 8999341;
            private _checkrank = [player] call ALSG_fnc_gangGetRank;
            ...
            _ctrlPromoteButton ctrlAddEventHandler ["ButtonClick", {
                ...
                if (_type isEqualTo 1) then {
                    ...
                    _playersrank = [player] call ALSG_fnc_gangGetRank;

                    private _newRank = _playerrank + 1;
                    private _maxRank = count(_ranks)-1;
                    if (_newRank isEqualTo _maxRank) exitWith {[format["%1 is the max rank they can be.",_playername],false,"slow"] call ALSG_fnc_notificationSystem};
                    if (_newRank isEqualTo _playersrank && _playersRank < 3) exitWith {["You can not promote to your level.",true,"slow"] call ALSG_fnc_notificationSystem;};
                    if (_newRank > _playersrank) exitWith {["You can not promote higher then your rank",true,"slow"] call ALSG_fnc_notificationSystem};
                    ...
                    [format["%1 has been promoted to %2", _playername, _rank],false,"slow"] call ALSG_fnc_notificationSystem;
                };
            }];
            ...
            _ctrlDemoteButton ctrlAddEventHandler ["ButtonClick", {
                ...
                if (_type isEqualTo 1) then {
                    ...
                    _rank = [player] call ALSG_fnc_gangGetRank;
                    if (_playerrank isEqualTo _rank) exitWith {["You can not demote a player with same rank as you.",true,"slow"] call ALSG_fnc_notificationSystem;};
                    if (_playerrank > _rank) exitWith {["You cannot demote people higher then you.",true,"slow"] call ALSG_fnc_notificationSystem;};

                    private _newRank = _playerrank - 1;
                    if (_newRank < 0) exitWith {[format["%1 is the lowest rank they can be.",_playername],false,"slow"] call ALSG_fnc_notificationSystem};
                    ...
                    _ctrlTree tvSetCurSel _selPath;
                    [format["%1 has been demoted from %2 to %3", _playername, _oldRankDisplay, _newRankDisplay],false,"slow"] call ALSG_fnc_notificationSystem;
                };
            }];
            ...
            _ctrlKickButton ctrlAddEventHandler ["ButtonClick", {
                ...
                if (_type isEqualTo 1) then {
                    ...
                    _rank = [player] call ALSG_fnc_gangGetRank;
                    if (_playerrank isEqualTo _rank) exitWith {["You can not kick a player with same rank as you.",true,"slow"] call ALSG_fnc_notificationSystem;};
                    if (_playerrank > _rank) exitWith {["You cannot kick people higher then you.",true,"slow"] call ALSG_fnc_notificationSystem;};

                    private _updated = _members - [_members select _index];
                    group player setVariable ["gang_members",_updated,true];

                    private _player = [_id] call ArmaLife_Lib_GetPlayerByUID;
                    ...
                    [format["%1 has been kicked from the gang by %2", _playername, name player],false,"slow"] remoteExec ["ALSG_fnc_notificationSystem", group player];
                };
            }];
        }
    };
...
private _membersOnline = _members select {
    _x params ["", "_uid"];
    [_uid] call ArmaLife_Lib_GetPlayerStatus;
};
...
_gangMembers = [_members, [], { _x select 2 }, "DESCEND"] call BIS_fnc_sortBy;
{
    ...
    private _isOnline = _uid call ArmaLife_Lib_GetPlayerStatus;
    ...
    private _rank = [_uid] call ArmaLife_Lib_GetPlayerStatus;
...
_ctrlButtonInvitePlayer ctrlAddEventHandler ["ButtonClick", {
    ...
    _ctrl ctrlEnable false;
    [_ctrlPlayerList] spawn ALSG_fnc_gangInvitePlayer;
}];
...
_buttonSetFreq ctrlAddEventHandler ["ButtonClick", {
    ...
    if(_gangfreq > 30 && _gangfreq < 35 || _gangfreq isEqualTo 40 || _gangfreq isEqualTo 60.3) exitWith {["30.1 - 34.9, 40, and 60.3 are blocked frequencies.",true,"slow"] call ALSG_fnc_notificationSystem;};
    if(_gangfreq isEqualTo 0) exitWith {["Frequency can not contain text.",true,"slow"] call ALSG_fnc_notificationSystem;};
    if(_gangfreq < 30 || _gangfreq > 512) exitWith {["Frequency range is 30-512.",true,"slow"] call ALSG_fnc_notificationSystem;};
    
    _checkfreq = [_gangfreq,_freqs] call TON_fnc_index;
    if (_checkfreq != -1) exitWith {["Frequency is already taken, please choose another one.",true,"slow"] call ALSG_fnc_notificationSystem;};
    ...
    closeDialog 0;
    missionNamespace setVariable ["ArmaLife_GangFreqs",_freqs,true];
    [format["You have set your new gang freq to %1", _gangfreq],false,"slow"] call ALSG_fnc_notificationSystem;
}];