  ...
  waitUntil {ArmaLife_Phone_CurrentlyOpen == "ArmaLife_Buttons"};
  ...
  private _mainButtons = "true" configClasses(missionconfigFile >> "ArmaLife_Apps");
  private _bottomButtons = "true" configClasses(missionconfigFile >> "ArmaLife_BottomIcons");
  ...
  private _heightSpacer = ((safeZoneW * 0.0105)*2);
  {
    ...
    if ((_condition isEqualto "") || (call compile _condition)) then {
      ...
      private _newControl = _display ctrlCreate ["ArmaLife_PhoneIconControlImage", _idc1, _controlGroup];
      private _newButton = _display ctrlCreate ["ArmaLife_PhoneIconControlButton", _idc2, _controlGroup];
      private _newText = _display ctrlCreate ["Life_RscStructuredText", _idc3, _controlGroup];

      _idc1 = _idc1 + 1;
      _idc2 = _idc2 + 1;
      _idc3 = _idc3 + 1;
      _newControl ctrlSetText format ["\ArmalifeCore\images\phone\icons\%1",(getText(_x >> "image"))];
  ...
  private _bottomControlsGroup = _display displayCtrl 1801;
  {
    ...
    private _newControl = _display ctrlCreate ["ArmaLife_PhoneIconControlImage", _bottomidc1, _bottomControlsGroup];
    private _newButton = _display ctrlCreate ["ArmaLife_PhoneIconControlButton", _bottomidc2, _bottomControlsGroup];
