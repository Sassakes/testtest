...
private _contacts = [ArmaLife_Phone_profileSettings];
{
...

{
    ...
    private _contactBackground = _display ctrlCreate["ArmaLife_PhoneRscPicture", _idcStart, _controlsGroup];
    _idcStart = _idcStart + 1;
    private _textCtrl = _display ctrlCreate["Life_RscStructuredText", _idcStart, _controlsGroup];
    _idcStart = _idcStart + 1;
    private _contactButton = _display ctrlCreate["ArmaLife_PhoneIconControlButton", _idcStart, _controlsGroup];
    ...
    private _config = (missionconfigfile >> "ArmaLife_Core_Settings");
    private _numberPrefix = getText(_config >> "phoneNumberPrefix");
    private _struct = "";
    if (((_x select 0) isEqualTo(ArmaLife_Phone_profileSettings select 0)) && ((_x select 1) isEqualTo(ArmaLife_Phone_profileSettings select 1)) && ((_x select 2) isEqualTo(ArmaLife_Phone_profileSettings select 2))) then {
        _struct = _struct + format["<t align='left' font='puristaMedium' shadow='0' size='%1'>My Number</t>", (((pixelH * 5) * ((getResolution select 1) / 1080)) * 110)];
        _struct = _struct + "<br/>";
    ...
    _contactBackground ctrlSetText format["\ArmalifeCore\images\phone\ui\ContactBackground1.paa"];
    if (_forEachIndex isEqualTo 0) then {
        _contactBackground ctrlSetText format["\ArmalifeCore\images\phone\ui\mynumberbackground.paa"];
    };
    ...
    if (_forEachIndex > 0) then {
        private _format = format["[%1] spawn ALSG_fnc_LoadContact", _forEachIndex];
...
[_allContactControls] spawn {
    ...
    waitUntil {
        ArmaLife_Phone_CurrentlyOpen != "ArmaLife_ContactsMain"
    };
    {
        ctrlDelete _x;
