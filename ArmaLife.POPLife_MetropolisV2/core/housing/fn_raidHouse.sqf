...
if (_houseInv isEqualTo [[],0]) exitWith {[localize "STR_House_Raid_Nothing",true,"slow"] call ALSG_fnc_notificationSystem;};
...
_value = 0;
{
    ...
    if (ITEM_ILLEGAL(_var) isEqualTo 1) then {
        if (!(ITEM_SELLPRICE(_var) isEqualTo -1)) then {
            ...
            _houseInvVal = _houseInvVal - (([_var] call ALSG_fnc_itemWeight) * _val);
...
if (_value > 0) then {
    [0,"STR_House_Raid_Successful",true,[[_value] call ALSG_fnc_numberText]] remoteExecCall ["ALSG_fnc_broadcast",RCLIENT];
    ...
    [localize "STR_House_Raid_NoIllegal",true,"slow"] call ALSG_fnc_notificationSystem;
};
