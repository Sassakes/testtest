...
[side _object] spawn {
    ...
    for "_i" from 0 to 1 step 0 do {
        ...
        if (!([_side,getPos player,30] call ALSG_fnc_nearUnits) && (player getVariable ["restrained",false]) && isNull objectParent player) exitWith {
            player setVariable ["restrained",false,true];
            player setVariable ["Escorting",false,true];
            player setVariable ["transporting",false,true];
            detach player;
			[localize "STR_Cop_ExcessiveRestrain",false,"slow"] call ALSG_fnc_notificationSystem;        
		};
    };
};

[format[localize "STR_Cop_Restrained",name _object],false,"slow"] call ALSG_fnc_notificationSystem;
