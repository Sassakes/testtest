...
_currentJail = "CarcelAlcatraz";
_allJails = MAIN_SETTINGS("Jail","Locations");
_myJail = _allJails >> _currentJail;
_myJailRelease = _allJails >> _currentJail >> "ReleaseArea";
[format["You have been arrested at the %1 by %2 for %3 Month(s), be safe!", getText(_myJail >> "name"), name _cop, _time],true,"slow"] call ALSG_fnc_notificationSystem;
...
[] spawn {
    while {life_is_arrested} do {
        ...
        [player] remoteExec ["ALSG_fnc_jailSaveTime",2];
    };
};

private _display = uiNamespace getVariable ["ArmaLife_JailHud",displayNull];
3 cutRsc ["ArmaLife_JailHud","PLAIN"];
...
["JAILHUD_HANDLE", "onEachFrame", {
	...
	_ctrlTextTitle ctrlSetText "Arrested at Carcel Alcatraz";
...
while {true} do {
    if (([player] call ALSG_fnc_playerInBuilding) && isSprintAllowed player) then {
...
switch (true) do {
    case (_bail): {
        ...
        [localize "STR_Jail_Paid",true,"slow"] call ALSG_fnc_notificationSystem;
    ...
    case (_esc): {
        life_is_arrested = false;
        player setVariable ["arrested", false, true];
        [localize "STR_Jail_EscapeSelf",true,"slow"] call ALSG_fnc_notificationSystem;
    ...
    case (alive player && !_esc && !_bail): {
        life_is_arrested = false;
        player setVariable ["arrested", false, true];
        [localize "STR_Jail_Released",true,"slow"] call ALSG_fnc_notificationSystem;
...
[false,"shank",ITEM_VALUE("shank")] call ALSG_fnc_handleInv;