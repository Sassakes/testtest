...
_fuelstations = nearestObjects [player, ['land_pop_gasolinera','Land_FuelStation_01_pump_F','Land_FuelStation_02_pump_F'],10];
if (_fuelstations isEqualTo []) exitWith {life_action_inUse = false;};
_vehicleList = nearestObjects [player, ["Car","air"], 10];
if (count _vehicleList < 1) exitWith {[localize "STR_NOTF_VehicleNear",true,"slow"] call ALSG_fnc_notificationSystem; life_action_inUse = false;};
if (!createDialog "Life_FuelStat") exitWith {};

_shop = MAIN_SETTINGS("Shops","Fuel");

[] spawn {waitUntil {!dialog}; life_action_inUse = false;};
ctrlSetText [20301,"Fuel Station"];
if (isNil "life_fuelPrices") then {
    life_fuelPrices = getNumber(_shop >> "fuel_cost");
};
