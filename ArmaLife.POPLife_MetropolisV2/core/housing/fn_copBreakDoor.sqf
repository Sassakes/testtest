...
if (isNil {(_house getVariable "house_owner")}) exitWith {[localize "STR_House_Raid_NoOwner",true,"slow"] call ALSG_fnc_notificationSystem;};

_uid = (_house getVariable "house_owner") select 0;
if (!([_uid] call ALSG_fnc_isUIDActive)) exitWith {[localize "STR_House_Raid_OwnerOff",true,"slow"] call ALSG_fnc_notificationSystem;};

_door = [_house] call ALSG_fnc_nearestDoor;
if (_door isEqualTo 0) exitWith {[localize "STR_Cop_NotaDoor",true,"slow"] call ALSG_fnc_notificationSystem;};
if ((_house getVariable [format ["bis_disabled_Door_%1",_door],0]) isEqualTo 0) exitWith {[localize "STR_House_Raid_DoorUnlocked",true,"slow"] call ALSG_fnc_notificationSystem;};

life_action_inUse = true;

//Setup the progress bar
_title = "Raiding House";
_icon = "\ArmalifeCore\images\displays\displayInteractionMenu\openDoor.paa";
...
_cP = 0.01;
_cpRate = _cP;
[format[localize "STR_House_Raid_NOTF",(_house getVariable "house_owner") select 1],true,"slow"] remoteExec ["ALSG_fnc_notificationSystem",RCLIENT];
