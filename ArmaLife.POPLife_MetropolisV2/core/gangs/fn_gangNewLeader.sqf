...
if ((lbCurSel 2621) isEqualTo -1) exitWith {[localize "STR_GNOTF_TransferSelect",true,"slow"] call ALSG_fnc_notificationSystem;};
_unit = call compile format ["%1",CONTROL_DATA(2621)];

if (isNull _unit) exitWith {};	
if (_unit == player) exitWith {[localize "STR_GNOTF_TransferSelf",true,"slow"] call ALSG_fnc_notificationSystem;};
...
if (_action) then {
    ...
    if (_unitID isEqualTo "") exitWith {[localize "STR_GNOTF_badUID",true,"slow"] call ALSG_fnc_notificationSystem;};	
    ...
    [localize "STR_GNOTF_TransferCancel",true,"slow"] call ALSG_fnc_notificationSystem;
};
[] remoteExec ["ALSG_fnc_loadGangMenu", _group];
