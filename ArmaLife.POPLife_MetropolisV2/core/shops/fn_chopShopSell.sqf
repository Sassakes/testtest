...
if ((time - life_action_delay) < 0.2) exitWith {[localize "STR_NOTF_ActionDelay",true,"slow"] call ALSG_fnc_notificationSystem; closeDialog 0;}; 
...
[localize "STR_Shop_ChopShopSelling",true,"slow"] call ALSG_fnc_notificationSystem;
life_action_inUse = true;

[_price] call ArmaLife_AC_Cash;
...
if (!isNil {(group player) getVariable ["gang_name",""]}) then {
    _oldamm = (group player) getVariable ["ArmaLife_ChoppedVehicles",0];
    _ammedit = _oldamm + 1;
    (group player) setVariable ["ArmaLife_ChoppedVehicles",_ammedit,true];
    
    if ((group player) getVariable ["ArmaLife_ChoppedVehicles",0] >= 9) then {
        ...
        if (_gangselect != 0) then {
            ...
            if (!_check) then {
                ...
                (group player) setVariable ["ArmaLife_ChoppedVehicles",nil,true];
            };
        };
    };
};