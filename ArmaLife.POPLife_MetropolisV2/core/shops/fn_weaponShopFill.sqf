...
tvClear _ctrlTree;
_path = ArmaLife_currentActiveShop;
...
if (count (getArray(_path >> "items")) > 0) then {
    {
        ...
        if ([_condition] call ALSG_fnc_levelCheck) then {
            private _itemClass = [
...
if (count (getArray(_path >> "weapons")) > 0) then {
    {
        ...
        if ([_condition] call ALSG_fnc_levelCheck) then {
            ...
            private _aComp = [_classname, true] call ALSG_fnc_weaponShopCompatible;
            {
                ...
                if !(_arrayComp isEqualTo []) then {
...

                    {
                        ...
                        private _index = [_compItem, _array apply {[toLower (_x param [0]), _x param [1], _x param [2], _x param [3], _x param [4]]}] call ALSG_fnc_weaponShopIndex;
                        (_array select _index) params ["_subClass", "_subDescription", "_subPrice", "_subCondition", "_subDisplay"];

                        if ([_subCondition] call ALSG_fnc_levelCheck) then {
