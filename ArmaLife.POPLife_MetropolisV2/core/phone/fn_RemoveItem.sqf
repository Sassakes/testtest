...
private _amountisnumber = [_amountStr] call ALSG_fnc_isNumber;
...
private _response = ["Remove Item?", _text] call ALSG_fnc_sendWarning;
if (_response) then {
	if ([false, _itemData, _amount] call ALSG_fnc_handleInv) then {
		[format["You have removed %2 %1(s) from your inventory.",_displayName,_amount],true,"slow"] call ALSG_fnc_notificationSystem;
		[3] call ALSG_fnc_updatePartial;
	} else {
		[format["You cannot remove %1 from you inventory.",_displayName],true,"slow"] call ALSG_fnc_notificationSystem;
	};
} else {
	[format["You have cancelled removing a %2 %1(s) from your inventory.",_displayName,_amount],true,"slow"] call ALSG_fnc_notificationSystem;
};
