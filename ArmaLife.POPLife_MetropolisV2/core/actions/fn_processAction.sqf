...
if (_type IN ["marijuana","cocaine","heroin"]) then {
    if (!((missionNamespace getVariable ["ArmaLife_Cartel",0]) > 0)) exitWith {};
    ...
    private _action = ["Cartel Fee", "This processer is owned by the cartel meaning they want a 25% fee out of your processing, would you like to pay this? If you hit no there could be bad results, choose wisly!", "YES", "NO", findDisplay 46, true] call ALSG_fnc_guiPrompt;
    if (!(isNull _group)) then {
        if (_action) then {
            private _cartelid = missionNamespace getVariable ["ArmaLife_Cartel",0];
            [false,((_oldItem select 0) select 0),_newamm] call ALSG_fnc_handleInv;

            [_price,_group,"Process"] remoteExec ["ArmaLife_ReciveGangProfit",2];
        } else {
            private _place = "";
            switch (_type) do { 
                case "marijuana": { 
            ...
            [player,_price,_place] remoteExec ["ArmaLife_DenyFee",_group];
            ["Bad choice, we will be handling you soon",true,"slow"] call ALSG_fnc_notificationSystem;
        };
    };
};
...
if (_exit) exitWith {life_is_processing = false; [localize "STR_NOTF_NotEnoughItemProcess",true,"slow"] call ALSG_fnc_notificationSystem; life_action_inUse = false;};
...
_oldItemWeight = 0;
{
    _weight = ([_x select 0] call ALSG_fnc_itemWeight) * (_x select 1);
...
if (_exit) exitWith {[localize "STR_Process_Weight",true,"slow"] call ALSG_fnc_notificationSystem; life_is_processing = false; life_action_inUse = false;};
...
_icon = "\ArmalifeCore\images\displays\displayInteractionMenu\inventory.paa";
...
if (CASH < _cost) exitWith {[format[localize "STR_Process_License",[_cost] call ALSG_fnc_numberText],true,"slow"] call ALSG_fnc_notificationSystem; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};
...
if (player getVariable ["inHostage",false]) exitWith {["You have been taken hostage, action canceled.",true,"slow"] call ALSG_fnc_notificationSystem; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};
if (player distance _vendor > 10) exitWith {[localize "STR_Process_Stay",true,"slow"] call ALSG_fnc_notificationSystem; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};
if (CASH < _cost) exitWith {[format[localize "STR_Process_License",[_cost] call ALSG_fnc_numberText],true,"slow"] call ALSG_fnc_notificationSystem; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};

"progressBar" cutText ["","PLAIN"];
if (_minimumConversions isEqualTo (_totalConversions call BIS_fnc_lowestNum)) then {[localize "STR_NOTF_ItemProcess",true,"slow"] call ALSG_fnc_notificationSystem;} else {[localize "STR_Process_Partial",true,"slow"] call ALSG_fnc_notificationSystem;};

[_cost,true] call ArmaLife_AC_Cash;
...
if (!isNil {(group player) getVariable ["gang_name",""]}) then {
    ...
    _oldamm = (group player) getVariable [format["ArmaLife_%1Processed",_type],0];
    _ammedit = _oldamm + _amm;
    (group player) setVariable [format["ArmaLife_%1Processed",_type],_ammedit,true];

    sleep 2;

    if ((group player) getVariable [format["ArmaLife_%1Processed",_type],0] >= _ammountNeeded) then {
        ...
        if (_gangselect != 0) then {
            ...
            if (!_check) then {
                ...
                (group player) setVariable [format["ArmaLife_%1Processed",_type],nil,true];
            };
        };
    };
};
...
{
    [false,(_x select 0),((_x select 1)*(_minimumConversions))] call ALSG_fnc_handleInv;
} count _oldItem;

{
    [true,(_x select 0),((_x select 1)*(_minimumConversions))] call ALSG_fnc_handleInv;
} count _newItem;