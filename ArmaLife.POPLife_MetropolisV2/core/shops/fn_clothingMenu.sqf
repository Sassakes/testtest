...
if !(isNull objectParent player) exitWith {[localize "STR_NOTF_ActionInVehicle",true,"slow"] call ALSG_fnc_notificationSystem};
...
if !(_exit) exitWith {[localize "STR_Shop_Veh_NoLicense",true,"slow"] call ALSG_fnc_notificationSystem;};
...
/* Open up the menu */
createDialog "ArmaLife_ClothingShop";
...
//Cop / Civ Pre Check
if (_shop in ["dive","reb","kart"] && {!(playerSide isEqualTo civilian)}) exitWith {[localize "STR_Shop_NotaCiv",true,"slow"] call ALSG_fnc_notificationSystem; closeDialog 0;};
if (_shop == "bruce" && playerSide isEqualTo independent) exitWith {["You are a medic you can not access this shop.",true,"slow"] call ALSG_fnc_notificationSystem; closeDialog 0;};
if (_shop == "cop" && {!(playerSide isEqualTo west)}) exitWith {[localize "STR_Shop_NotaCop",true,"slow"] call ALSG_fnc_notificationSystem; closeDialog 0;};
if (_shop == "dive" && {!(missionNamespace getVariable LICENSE_VARNAME("dive","civ"))}) exitWith {[localize "STR_Shop_NotaDive",true,"slow"] call ALSG_fnc_notificationSystem; closeDialog 0;};
...
if (isClass(missionConfigFile >> "Licenses" >> life_clothing_store)) then {
    ...
    if !(LICENSE_VALUE(life_clothing_store,_flag)) exitWith {
        [format[localize "STR_Shop_YouNeed",localize _displayName],true,"slow"] call ALSG_fnc_notificationSystem;
        closeDialog 0;
    };
};
...
_ctrlTree ctrlAddEventHandler ["TreeSelChanged", { 
    ...
    if (_selection isEqualTo []) exitWith {[localize "STR_Shop_NoSelection",true,"slow"] call ALSG_fnc_notificationSystem;};
    if (isNull _control) exitWith {[localize "STR_Shop_NoDisplay",true,"slow"] call ALSG_fnc_notificationSystem;};
    ...
    if (_data isEqualTo "NONE") then {
        ...
        [_item,false] call ALSG_fnc_handleItem;
    } else {
        [_data,true,nil,nil,nil,nil,nil,true] call ALSG_fnc_handleItem;
    };

    life_cMenu_lock = false;
    _price ctrlSetStructuredText parseText format ["<t color='#8cff9b'>$%1</t>",[(_control tvValue _selection)] call ALSG_fnc_numberText];
    ...
    _total ctrlSetStructuredText parseText format ["<t color='#8cff9b'>$%1</t>",[_totalPrice] call ALSG_fnc_numberText];
}];
...
if (isNil "life_clothesPurchased") exitWith {
    ...
    if !(backpack player isEqualTo "") then {
        if (life_oldBackpack isEqualTo "") then {
            ...
            if (count life_oldBackpackItems > 0) then {
                {
                    [_x,true,true] call ALSG_fnc_handleItem;
                    true
    ...
    if (count life_oldUniformItems > 0) then {
        {
            [_x,true,false,false,true] call ALSG_fnc_handleItem;
            true
