...
waitUntil {ArmaLife_Phone_CurrentlyOpen isEqualTo "ArmaLife_MessagesMain"};
	...
	
{
  ...
  private _messageListBackgroundControl = _display ctrlCreate ["ArmaLife_PhoneRscPicture", _someIDC, _controlGroup];   _someIDC = _someIDC + 1;
  _messageListBackgroundControl ctrlEnable false;
  private _messageTextControl = _display ctrlCreate ["ArmaLife_PhoneStructuredText", _someIDC, _controlGroup];              _someIDC = _someIDC + 1;
  _messageTextControl ctrlEnable false;
  private _messageListButton = _display ctrlCreate ["ArmaLife_PhoneIconControlButton", _someIDC, _controlGroup];            _someIDC = _someIDC + 1;
  _messageListButton ctrlAddEventHandler ["ButtonClick", format ["['%1'] spawn ALSG_fnc_LoadMessages", _number]];	
  ...
  _messageTextControl ctrlSetTextColor [0, 0, 0, 1];	
   _messageListBackgroundControl ctrlSetText "\ArmalifeCore\images\phone\dialogImages\MessageListBackground.paa";	
...
} forEach ArmaLife_Phone_Messages;

[_allMessageControls] spawn {
  params ["_allMessageControls"];

  waitUntil {ArmaLife_Phone_CurrentlyOpen != "ArmaLife_MessagesMain"};
