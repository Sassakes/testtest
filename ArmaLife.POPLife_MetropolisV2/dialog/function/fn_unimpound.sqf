...
[_retreivePrice,_vid,_className,_vendor] spawn {
    ...
    private _action = ["Retreive Vehicle", format["Are you sure you want to retrieve this vehicle for $%1?",[_retreivePrice] call ALSG_fnc_numberText], "YES", "NO", findDisplay 46, true] call ALSG_fnc_guiPrompt;
    if (_action) then {
        closeDialog 0;
        if (BANK < _retreivePrice) exitWith {[format[localize "STR_Garage_CashError",[_retreivePrice] call ALSG_fnc_numberText],true,"slow"] call ALSG_fnc_notificationSystem;};

        if (life_garage_sp isEqualTo "NONE") exitWith {["No spawnpoint available, please let a developer know this shop has no spawn.",true,"slow"] call ALSG_fnc_notificationSystem;};
        if (life_garage_sp isEqualType []) then {
            [_vid,(life_garage_sp select 0), (life_garage_sp select 1), _retreivePrice] call ArmaLife_spawnVehicle;
        } else {
            if (life_garage_sp isEqualTo "player_garage") then {
                [_className,_vendor,_retreivePrice,_vid] spawn {
                    ...
                    ["To place your vehicle please press space, to cancel this press F1",true,"slow"] call ALSG_fnc_notificationSystem;
                    ...
                    garageSpawnEh = (findDisplay 46) displayAddEventHandler['KeyUp', {
                        if (_this select 1 == 0x39) then {
                            ...
                            if (_this select 1 == 0x3B) then {
                                ...
                                garageExited = true;
                                [0] call ArmaLife_finishVehSpawn;
                            };
                        };
                    }];
                    ...
                    [_vid, _pos, _dir, _retreivePrice] call ArmaLife_spawnVehicle;
                    ...
                    if (player distance _startposition > _distance) then {
                        ["You have walked to far, dropping vehicle.",true,"slow"] call ALSG_fnc_notificationSystem;
                    };
                };
            } else {
                if (life_garage_sp in ["medic_spawn_1","medic_spawn_2","medic_spawn_3"]) then {
                    [_vid, life_garage_sp, 0, _retreivePrice] call ArmaLife_spawnVehicle;
                } else {
                    [_vid, (getMarkerPos life_garage_sp), (markerDir life_garage_sp), _retreivePrice] call ArmaLife_spawnVehicle;
                };
            };
        };
    };
