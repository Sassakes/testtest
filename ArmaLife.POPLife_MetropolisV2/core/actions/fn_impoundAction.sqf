...
if (_vehicle getVariable "NPC") exitWith {[localize "STR_NPC_Protected",true,"slow"] call ALSG_fnc_notificationSystem;};
if (life_action_inUse) exitWith {["You are in an action already.",true,"slow"] call ALSG_fnc_notificationSystem;};
...
_player = [((_vehicleData select 0) select 0)] call ArmaLife_Lib_GetPlayerByUID;

[format[localize "STR_NOTF_BeingImpounded",((_vehicleData select 0) select 1),_vehicleName],false,"slow"] remoteExecCall ["ALSG_fnc_notificationSystem",_player];
life_action_inUse = true;

//Setup the progress bar
_title = "Impounding Vehicle";
_icon = "\ArmalifeCore\images\displays\displayInteractionMenu\impound.paa";
...
if (player distance _vehicle > 10) exitWith {[localize "STR_NOTF_ImpoundingCancelled",true,"slow"] call ALSG_fnc_notificationSystem; life_action_inUse = false;};
...
if (count crew _vehicle isEqualTo 0) then {
    ...
    if (_vehicle isKindOf "Car") then {
      [_vehicle,player] call ArmaLife_ImpoundVehicle;
    } else {
      ...
      if (_vehicle in life_vehicles) then {
        [format[localize "STR_NOTF_OwnImpounded",[_value] call ALSG_fnc_numberText,_type],true,"slow"] call ALSG_fnc_notificationSystem;
        [_value,true] call ArmaLife_AC_Bank;
      } else {
        [format[localize "STR_NOTF_Impounded",_type,[_value] call ALSG_fnc_numberText],true,"slow"] call ALSG_fnc_notificationSystem;
    ...
    [localize "STR_NOTF_ImpoundingCancelled",true,"slow"] call ALSG_fnc_notificationSystem;
};
