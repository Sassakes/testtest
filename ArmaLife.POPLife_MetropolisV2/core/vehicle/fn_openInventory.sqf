...
if (isNull _vehicle || !(_vehicle isKindOf "Car" || _vehicle isKindOf "Air" || _vehicle isKindOf "Ship" || _vehicle isKindOf "Box_IND_Grenades_F" || _vehicle isKindOf "B_supplyCrate_F" || _vehicle isKindOf "Land_jbad_chest" || _vehicle isKindOf "B_CargoNet_01_ammo_F" || _vehicle isKindOf "Land_CargoBox_V1_F")) exitWith {}; //Either a null or invalid vehicle type.
if (life_is_processing) exitWith {["You cannot access a vehicle while processing.",true,"slow"] call ALSG_fnc_notificationSystem;};
if ((_vehicle getVariable ["trunk_in_use",false])) exitWith {[localize "STR_MISC_VehInvUse",true,"slow"] call ALSG_fnc_notificationSystem;};
    
_unitsnear = (nearestObjects[_vehicle, ["Man"], 15]) arrayIntersect playableUnits;
if(count _unitsnear > 1) exitWith {["Sorry, you cannot open your inventory to your vehicle when players are nearby!",true,"slow"] call ALSG_fnc_notificationSystem;};

if (_vehicle isKindOf "Land_CargoBox_V1_F" && !(_vehicle getVariable ["bankOpen",false])) exitWith {
    ["Safe is locked, you must open this with a blasting charge",true,"slow"] call ALSG_fnc_notificationSystem;
};

_vehicle setVariable ["trunk_in_use",true,true];
_vehicle setVariable ["trunk_in_use_by",player,true];
if (!createDialog "ArmaLife_VehicleInventory") exitWith {[localize "STR_MISC_DialogError",true,"slow"] call ALSG_fnc_notificationSystem;};
...
_veh_data = [_vehicle] call ALSG_fnc_vehicleWeight;
if (_veh_data select 0 isEqualTo -1) exitWith {
    [_vehicle, player] remoteExecCall ["ArmaLife_AC_CloseTrunk", 2];
    closeDialog 0;
    _vehicle setVariable ["trunk_in_use",false,true];
    [localize "STR_MISC_NoStorageVeh",true,"slow"] call ALSG_fnc_notificationSystem;
};
...
[_vehicle] call ALSG_fnc_vehInventory;
life_trunk_vehicle = _vehicle;

_vehicle spawn {
    waitUntil {isNull (findDisplay 3500)};
    _this setVariable ["trunk_in_use",false,true];
    [_this, player] remoteExecCall ["ArmaLife_AC_CloseTrunk", 2];
