...
*/
if (life_is_arrested) exitWith {[] call ALSG_fnc_respawned;};
if (life_respawned) then {[] call ALSG_fnc_respawned;};

closeDialog 0;
createDialog "ArmaLife_SpawnMenu";

private _display = findDisplay 8999720;
private _ctrlButtonLobby = _display displayCtrl 10;
_ctrlButtonLobby ctrlSetEventHandler ["ButtonClick", "endMission 'ArmaLife_End'"];
_display displaySetEventHandler ["keyDown","_this call ALSG_fnc_displayHandler"];
...

{
    ...
    private _flag = [_conditions] call ALSG_fnc_levelCheck;
  ...
  if (count life_houses > 0) then {
    {
		...
		_ctrlTree tvSetPicture [[_treeEntryProperties, _index], "\ArmalifeCore\images\displays\displaySpawns\TreeIcons\villa.paa"];
...
[_array select 2] spawn ALSG_fnc_spawnCamera;