...
switch (_btn) do {
	case 1 : {
		if (life_settings_tagson) then {
			...
			[ALSG_ID_VehicleTags,"onEachFrame"] call BIS_fnc_removeStackedEventHandler;
			...
			ALSG_ID_VehicleTags = ["ALSG_VehicleTags","onEachFrame",{{_p = _x select 0; _pos = visiblePositionASL _p; _pos set [2, ((_p modelToWorld [0,0,0]) select 2) + 1]; drawIcon3D ["", [0.267,0.549,1,1],_pos, 1, 1, 0, _x select 1, 1, 0.03, "PuristaMedium", "center", false];} foreach (missionNameSpace getVariable ["ALSG_TagsArray",[]]);}] call BIS_fnc_addStackedEventHandler;
			LIFE_ID_PlayerTags = ["LIFE_PlayerTags","onEachFrame",{{_p = _x select 0; _pos = visiblePositionASL _p; _pos set [2, ((_p modelToWorld [0,0,0]) select 2) + 2]; drawIcon3D ["", _x select 2,_pos, 0.2, 0.2, 45, _x select 1, 1, 0.03, "PuristaBold"];} foreach (missionNameSpace getVariable ["ArmaLife_TagsArray",[]]);}] call BIS_fnc_addStackedEventHandler;
		};
	};
